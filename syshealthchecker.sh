...
ip_add=`ip addr | grep "inet " | grep -v "127.0.0.1" | awk '{print $2}' | cut -f1 -d'/'`
num_proc=`ps -ef | wc -l`
...
cpu_free=`top -n 1 | awk 'NR>7 {print$9}' | sed -n '1p'`
...
top -n 1 | head -17 | tail -11 | awk '{print $1, $2, $9, $12}' | grep -v PID > /tmp/cpustat.txt
...
echo "<h3><legend>Script authored by Harun & Mostakim</legend></h3>" >> $html
echo "</center>" >> $html
echo "</fieldset>" >> $html
echo "<br>" >> $html
echo "<center>" >> $html
...
cat $html | mail -s "`hostname` - Daily System Health Report" -a "MIME-Version: 1.0" -a "Content-Type: text/html" -a "From: Tutul <root@tutul.com>" $email_add
